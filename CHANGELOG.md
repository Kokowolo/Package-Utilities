...
* `GameIO` to handling loading/saving of game byte files
### Changed
* `LateUpdate` usage within `WorldCursor`
* `WorldCursor`'s function naming
* namespace `WorldCursorManager` resided in (was previously global)
...
### Changed
* several `LayerMaskExtensions` methods to be slightly simpler

## [0.0.148] - 2026-01-29
### Added
* setter to `ScriptableObjectSingleton.Instance`
### Changed
* naming convention of all `PropertyDrawers` to now end with just `Drawer`
...
## [0.0.146] - 2026-01-22
### Added
* AI generated functions to assist in `SerializedProperty` grabbing and display within the `Editor`
* error handling to `EditorExtensions` when `SerializedProperty.boxedValue` is not accessible
